...
################ Normality tests for the Pmax data: 

#### Kolmogorov-Smirnov test:

# K-S test for SITE groups:
ks.test(FIRe_ave[FIRe_ave$SITE=="IUI_S",]$Pmax, "pnorm")
ks.test(FIRe_ave[FIRe_ave$SITE=="IUI_N",]$Pmax, "pnorm")
ks.test(FIRe_ave[FIRe_ave$SITE=="JG",]$Pmax, "pnorm")

# K-S test for DEPTH groups:
ks.test(FIRe_ave[FIRe_ave$DEPTH=="45",]$Pmax, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="35",]$Pmax, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="25",]$Pmax, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="15",]$Pmax, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="5",]$Pmax, "pnorm")

# K-S test for Species groups:
ks.test(FIRe_ave[FIRe_ave$Species=="Stylophora",]$Pmax, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Porites",]$Pmax, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Pocillopora",]$Pmax, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Leptoseris",]$Pmax, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Alveopora",]$Pmax, "pnorm")
...
############### ART-Anova test for the Pmax parameter for the 5 corals:
...
################### STYLOPHORA:
### Aligned ranks anova:
model_Stylo_Pmax = art(Pmax ~ DEPTH+(1|SITE) , data = FIRe_ave_Stylophora) # +(1|SITE) took off the site factor
### Check the success of the procedure:
model_Stylo_Pmax

### Conduct ANOVA:
anova(model_Stylo_Pmax)

# Posthoc pairwise comparison for the DEPTH:
Stylo_DEPTH_posthoc = art.con(model_Stylo_Pmax, "DEPTH", adjust="tukey")
Stylo_DEPTH_posthoc
...
################### PORITES:
### Aligned ranks anova:
model_Porites_Pmax = art(Pmax ~ DEPTH+(1|SITE) , data = FIRe_ave_Porites)
### Check the success of the procedure:
model_Porites_Pmax

### Conduct ANOVA:
anova(model_Porites_Pmax)

# Posthoc pairwise comparison for the DEPTH:
Porites_DEPTH_posthoc = art.con(model_Porites_Pmax, "DEPTH", adjust="tukey")
...
################### POCILLOPORA:
### Aligned ranks anova:
model_Pocill_Pmax = art(Pmax ~ DEPTH+(1|SITE) , data = FIRe_ave_Pocillopora)
### Check the success of the procedure:
model_Pocill_Pmax

### Conduct ANOVA:
anova(model_Pocill_Pmax)

# Posthoc pairwise comparison for the DEPTH:
Pocill_DEPTH_posthoc = art.con(model_Pocill_Pmax, "DEPTH", adjust="tukey")
...
################### LEPTOSERIS:
### Aligned ranks anova:
model_Lepto_Pmax = art(Pmax ~ DEPTH+(1|SITE) , data = FIRe_ave_Leptoseris)
### Check the success of the procedure:
model_Lepto_Pmax

### Conduct ANOVA:
anova(model_Lepto_Pmax)

# Posthoc pairwise comparison for the DEPTH:
Lepto_DEPTH_posthoc = art.con(model_Lepto_Pmax, "DEPTH", adjust="tukey")
Lepto_DEPTH_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### ALVEOPORA:
### Aligned ranks anova:
model_Alveo_Pmax = art(Pmax ~ DEPTH, data = FIRe_ave_Alveopora)
### Check the success of the procedure:
model_Alveo_Pmax

### Conduct ANOVA:
anova(model_Alveo_Pmax)  ## Non significant!


############# Plotting Pmax: 

# Calculates the 25th percentile=מאון (1st quartile) and the 75th percentile (3rd quartile) 
# of the Pmax column in the FIRe_ave DF:
quartiles <- quantile(FIRe_ave$Pmax, probs = c(.25,.75))
# Calculates the interquartile range (IQR) of the Pmax column 
# which is the difference between the 3rd and 1st quartiles:
IQR_cor <- IQR(FIRe_ave$Pmax)
...
Pmax_no_outlier <- subset(FIRe_ave, FIRe_ave$Pmax >lower & FIRe_ave$Pmax< upper)

Pmax <- ggplot(data = Pmax_no_outlier,aes(x= DEPTH, y=Pmax, fill= Species))+
                 geom_boxplot() + theme_classic() + stat_boxplot(geom = 'errorbar', width = 0.75, position = "dodge") +
                 labs(x="Depth in meters", y="Pmax (e/sec)") + # axes labels 
                 theme_bw() + 
...
############### ART-Anova test for the Pmax parameter for the 5 depth-points:

################### 5m:
### Aligned ranks anova:
model_5_Pmax = art(Pmax ~ Species+(1|SITE) , data = FIRe_ave_5) # +(1|SITE) took off the site factor
### Check the success of the procedure:
model_5_Pmax
...
# Posthoc pairwise comparison for the DEPTH:
Species_5_posthoc = art.con(model_5_Pmax, "Species", adjust="tukey")
Species_5_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### 15m:
### Aligned ranks anova:
model_15_Pmax = art(Pmax ~ Species+(1|SITE) , data = FIRe_ave_15) # +(1|SITE) took off the site factor
### Check the success of the procedure:
model_15_Pmax
...
# Posthoc pairwise comparison for the DEPTH:
Species_15_posthoc = art.con(model_15_Pmax, "Species", adjust="tukey")
Species_15_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### 25m:
### Aligned ranks anova:
model_25_Pmax = art(Pmax ~ Species, data = FIRe_ave_25) 
### Check the success of the procedure:
model_25_Pmax
...
# Posthoc pairwise comparison for the DEPTH:
Species_25_posthoc = art.con(model_25_Pmax, "Species", adjust="tukey")
Species_25_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### 35m:
### Aligned ranks anova:
model_35_Pmax = art(Pmax ~ Species, data = FIRe_ave_35) 
### Check the success of the procedure:
model_35_Pmax
...
# Posthoc pairwise comparison for the DEPTH:
Species_35_posthoc = art.con(model_35_Pmax, "Species", adjust="tukey")
Species_35_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### 45m:
### Aligned ranks anova:
model_45_Pmax = art(Pmax ~ Species, data = FIRe_ave_45) 
### Check the success of the procedure:
model_45_Pmax
...
# Posthoc pairwise comparison for the DEPTH:
Species_45_posthoc = art.con(model_45_Pmax, "Species", adjust="tukey")
Species_45_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
Pmax <- ggplot(data = Pmax_no_outlier,aes(x= Species, y=Pmax, fill= Species))+
  geom_boxplot() + theme_classic() + stat_boxplot(geom = 'errorbar', width = 0.75, position = "dodge") +
  labs(x="Corals", y="Pmax") + # axes labels 
  theme_bw() + 
  theme(legend.position = "none",
        axis.title.x = element_text(colour="Black", size=16),
        axis.title.y = element_text(colour="Black", size=16),

...
################ Normality tests for the p data: 

#### Kolmogorov-Smirnov test:

# K-S test for SITE groups:
ks.test(FIRe_ave[FIRe_ave$SITE=="IUI_S",]$p, "pnorm")
ks.test(FIRe_ave[FIRe_ave$SITE=="IUI_N",]$p, "pnorm")
ks.test(FIRe_ave[FIRe_ave$SITE=="JG",]$p, "pnorm")

# K-S test for DEPTH groups:
ks.test(FIRe_ave[FIRe_ave$DEPTH=="45",]$p, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="35",]$p, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="25",]$p, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="15",]$p, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="5",]$p, "pnorm")

# K-S test for Species groups:
ks.test(FIRe_ave[FIRe_ave$Species=="Stylophora",]$p, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Porites",]$p, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Pocillopora",]$p, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Leptoseris",]$p, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Alveopora",]$p, "pnorm")
...
############### ART-Anova test for the p parameter for the 5 corals:
...
################### STYLOPHORA:
### Aligned ranks anova:
model_Stylo_p = art(p ~ DEPTH+(1|SITE) , data = FIRe_ave_Stylophora) # +(1|SITE) took off the site factor
...
Stylo_DEPTH_posthoc = art.con(model_Stylo_p, "DEPTH", adjust="tukey")
Stylo_DEPTH_posthoc
...
################### PORITES:
### Aligned ranks anova:
model_Porites_p = art(p ~ DEPTH+(1|SITE) , data = FIRe_ave_Porites)
### Check the success of the procedure:
model_Porites_p

### Conduct ANOVA:
anova(model_Porites_p) 

# Posthoc pairwise comparison for the DEPTH:
Porites_DEPTH_posthoc = art.con(model_Porites_p, "DEPTH", adjust="tukey")
...
################### POCILLOPORA:
### Aligned ranks anova:
model_Pocill_p = art(p ~ DEPTH+(1|SITE) , data = FIRe_ave_Pocillopora)
### Check the success of the procedure:
model_Pocill_p

### Conduct ANOVA:
anova(model_Pocill_p)

# Posthoc pairwise comparison for the DEPTH:
Pocill_DEPTH_posthoc = art.con(model_Pocill_p, "DEPTH", adjust="tukey")
...
################### LEPTOSERIS:
### Aligned ranks anova:
model_Lepto_p = art(p ~ DEPTH+(1|SITE) , data = FIRe_ave_Leptoseris)
### Check the success of the procedure:
model_Lepto_p
...
# Posthoc pairwise comparison for the DEPTH:
Lepto_DEPTH_posthoc = art.con(model_Lepto_p, "DEPTH", adjust="tukey")
Lepto_DEPTH_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### ALVEOPORA:
### Aligned ranks anova:
model_Alveo_p = art(p ~ DEPTH , data = FIRe_ave_Alveopora)
### Check the success of the procedure:
model_Alveo_p

### Conduct ANOVA:
anova(model_Alveo_p)  ## Non significant!


############# Plotting p: 

# Calculates the 25th percentile=מאון (1st quartile) and the 75th percentile (3rd quartile) 
# of the p column in the FIRe_ave DF:
quartiles <- quantile(FIRe_ave$p, probs = c(.25,.75))
# Calculates the interquartile range (IQR) of the p column 
# which is the difference between the 3rd and 1st quartiles:
IQR_cor <- IQR(FIRe_ave$p)
...
p_no_outlier <- subset(FIRe_ave, FIRe_ave$p >lower & FIRe_ave$p< upper)
...
p <- ggplot(data = p_no_outlier,aes(x= DEPTH, y=p, fill= Species))+
  ...
  labs(x="Depth in meters", y="Connectivity p") + # axes labels 
  theme_bw() + 
...
############### ART-Anova test for the p parameter for the 5 depth-points:

################### 5m:
### Aligned ranks anova:
model_5_p = art(p ~ Species+(1|SITE) , data = FIRe_ave_5) # +(1|SITE) took off the site factor
### Check the success of the procedure:
model_5_p
...
# Posthoc pairwise comparison for the DEPTH:
Species_5_posthoc = art.con(model_5_p, "Species", adjust="tukey")
Species_5_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### 15m:
### Aligned ranks anova:
model_15_p = art(p ~ Species+(1|SITE) , data = FIRe_ave_15) # +(1|SITE) took off the site factor
### Check the success of the procedure:
model_15_p

### Conduct ANOVA:
anova(model_15_p)   # Non-significant!


################### 25m:
### Aligned ranks anova:
model_25_p = art(p ~ Species, data = FIRe_ave_25) 
### Check the success of the procedure:
model_25_p
...
# Posthoc pairwise comparison for the DEPTH:
Species_25_posthoc = art.con(model_25_p, "Species", adjust="tukey")
Species_25_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### 35m:
### Aligned ranks anova:
model_35_p = art(p ~ Species, data = FIRe_ave_35) 
### Check the success of the procedure:
model_35_p

### Conduct ANOVA:
anova(model_35_p)  # Non-significant!


################### 45m:
### Aligned ranks anova:
model_45_p = art(p ~ Species, data = FIRe_ave_45) 
### Check the success of the procedure:
model_45_p
...
# Posthoc pairwise comparison for the DEPTH:
Species_45_posthoc = art.con(model_45_p, "Species", adjust="tukey")
Species_45_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
p <- ggplot(data = p_no_outlier,aes(x= Species, y=p, fill= Species))+
  geom_boxplot() + theme_classic() + stat_boxplot(geom = 'errorbar', width = 0.75, position = "dodge") +
  labs(x="Corals", y="Connectivity (p)") + # axes labels 
  theme_bw() + 
  theme(legend.position = "none",
        axis.title.x = element_text(colour="Black", size=16),
        axis.title.y = element_text(colour="Black", size=16),

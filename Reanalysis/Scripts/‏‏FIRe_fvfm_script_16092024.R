...
ks.test(FIRe_ave[FIRe_ave$SITE=="IUI_S",]$fv_fm, "pnorm")
ks.test(FIRe_ave[FIRe_ave$SITE=="IUI_N",]$fv_fm, "pnorm")
ks.test(FIRe_ave[FIRe_ave$SITE=="JG",]$fv_fm, "pnorm")

# K-S test for DEPTH groups:
ks.test(FIRe_ave[FIRe_ave$DEPTH=="45",]$fv_fm, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="35",]$fv_fm, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="25",]$fv_fm, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="15",]$fv_fm, "pnorm")
ks.test(FIRe_ave[FIRe_ave$DEPTH=="5",]$fv_fm, "pnorm")

# K-S test for Species groups:
ks.test(FIRe_ave[FIRe_ave$Species=="Stylophora",]$fv_fm, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Porites",]$fv_fm, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Pocillopora",]$fv_fm, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Leptoseris",]$fv_fm, "pnorm")
ks.test(FIRe_ave[FIRe_ave$Species=="Alveopora",]$fv_fm, "pnorm")
...
# ART-Anova test for the fv/fm parameter for the 5 corals
...
################### STYLOPHORA:
### Aligned ranks anova:
model_Stylo_fvfm = art(fv_fm ~ DEPTH+(1|SITE) , data = FIRe_ave_Stylophora) # +(1|SITE) took off the site factor
### Check the success of the procedure:
model_Stylo_fvfm

### Conduct ANOVA:
anova(model_Stylo_fvfm)

# Posthoc pairwise comparison for the DEPTH:
Stylo_DEPTH_posthoc = art.con(model_Stylo_fvfm, "DEPTH", adjust="tukey")
Stylo_DEPTH_posthoc
...
################### PORITES:
### Aligned ranks anova:
model_Porites_fvfm = art(fv_fm ~ DEPTH+(1|SITE) , data = FIRe_ave_Porites)
### Check the success of the procedure:
model_Porites_fvfm

### Conduct ANOVA:
anova(model_Porites_fvfm)

# Posthoc pairwise comparison for the DEPTH:
Porites_DEPTH_posthoc = art.con(model_Porites_fvfm, "DEPTH", adjust="tukey")
Porites_DEPTH_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
Sum_Porites_DEPTH = as.data.frame(Porites_DEPTH_posthoc)
# Finding the statistical group letter: 
cldList(p.value ~ contrast, data=Sum_Porites_DEPTH)


################### POCILLOPORA:
### Aligned ranks anova:
model_Pocill_fvfm = art(fv_fm ~ DEPTH+(1|SITE) , data = FIRe_ave_Pocillopora)
### Check the success of the procedure:
model_Pocill_fvfm

### Conduct ANOVA:
anova(model_Pocill_fvfm)

# Posthoc pairwise comparison for the DEPTH:
Pocill_DEPTH_posthoc = art.con(model_Pocill_fvfm, "DEPTH", adjust="tukey")
Pocill_DEPTH_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
Sum_Pocill_DEPTH = as.data.frame(Pocill_DEPTH_posthoc)
# Finding the statistical group letter: 
cldList(p.value ~ contrast, data=Sum_Pocill_DEPTH)


################### LEPTOSERIS:
### Aligned ranks anova:
model_Lepto_fvfm = art(fv_fm ~ DEPTH+(1|SITE) , data = FIRe_ave_Leptoseris)
### Check the success of the procedure:
model_Lepto_fvfm

### Conduct ANOVA:
anova(model_Lepto_fvfm)

# Posthoc pairwise comparison for the DEPTH:
Lepto_DEPTH_posthoc = art.con(model_Lepto_fvfm, "DEPTH", adjust="tukey")
Lepto_DEPTH_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
Sum_Lepto_DEPTH = as.data.frame(Lepto_DEPTH_posthoc)
# Finding the statistical group letter: 
cldList(p.value ~ contrast, data=Sum_Lepto_DEPTH)


################### ALVEOPORA:
### Aligned ranks anova:
model_Alveo_fvfm = art(fv_fm ~ DEPTH , data = FIRe_ave_Alveopora) # No need: +(1|SITE) cause Alveo only in 1 site! 
### Check the success of the procedure:
model_Alveo_fvfm

### Conduct ANOVA:
anova(model_Alveo_fvfm)  ## Non significant!


############# Plotting fv_fm by facet coral: 

# Calculates the 25th percentile=מאון (1st quartile) and the 75th percentile (3rd quartile) 
# of the fv_fm column in the FIRe_ave DF:
quartiles <- quantile(FIRe_ave$fv_fm, probs = c(.25,.75))
# Calculates the interquartile range (IQR) of the fv_fm column 
# which is the difference between the 3rd and 1st quartiles:
IQR_cor <- IQR(FIRe_ave$fv_fm)
...
fvfm_no_outlier <- subset(FIRe_ave, FIRe_ave$fv_fm >lower & FIRe_ave$fv_fm< upper)
...
fvfm <- ggplot(data = fvfm_no_outlier,aes(x= DEPTH, y=fv_fm, fill= Species))+
  ...
  labs(x="Depth in meters", y="Fv’/Fm’") + # axes labels 
  theme_bw() + 
...
# Note: I removed plot title from labs(title="Quantum yield of photochemistry in PSII", ) 
...
############### ART-Anova test for the fv/fm parameter for the 5 depth-points:

################### 5m:
### Aligned ranks anova:
model_5_fvfm = art(fv_fm ~ Species+(1|SITE) , data = FIRe_ave_5) # +(1|SITE) took off the site factor
### Check the success of the procedure:
model_5_fvfm
...
# Posthoc pairwise comparison for the DEPTH:
Species_5_posthoc = art.con(model_5_fvfm, "Species", adjust="tukey")
Species_5_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### 15m:
### Aligned ranks anova:
model_15_fvfm = art(fv_fm ~ Species+(1|SITE) , data = FIRe_ave_15) # +(1|SITE) took off the site factor
### Check the success of the procedure:
model_15_fvfm
...
# Posthoc pairwise comparison for the DEPTH:
Species_15_posthoc = art.con(model_15_fvfm, "Species", adjust="tukey")
Species_15_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### 25m:
### Aligned ranks anova:
model_25_fvfm = art(fv_fm ~ Species, data = FIRe_ave_25) 
### Check the success of the procedure:
model_25_fvfm
...
# Posthoc pairwise comparison for the DEPTH:
Species_25_posthoc = art.con(model_25_fvfm, "Species", adjust="tukey")
Species_25_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### 35m:
### Aligned ranks anova:
model_35_fvfm = art(fv_fm ~ Species, data = FIRe_ave_35) 
### Check the success of the procedure:
model_35_fvfm
...
# Posthoc pairwise comparison for the DEPTH:
Species_35_posthoc = art.con(model_35_fvfm, "Species", adjust="tukey")
Species_35_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
################### 45m:
### Aligned ranks anova:
model_45_fvfm = art(fv_fm ~ Species, data = FIRe_ave_45) 
### Check the success of the procedure:
model_45_fvfm
...
# Posthoc pairwise comparison for the DEPTH:
Species_45_posthoc = art.con(model_45_fvfm, "Species", adjust="tukey")
Species_45_posthoc
### For Tukey-adjusted p-values, use adjust="tukey" otherwise use: "none"
...
fvfm <- ggplot(data = fvfm_no_outlier,aes(x= Species, y=fv_fm, fill= Species))+
  geom_boxplot() + theme_classic() + stat_boxplot(geom = 'errorbar', width = 0.75, position = "dodge") +
  labs(x="Corals", y="Fv’/Fm’") + # axes labels 
  theme_bw() + 
  theme(legend.position = "none",
        axis.title.x = element_text(colour="Black", size=16),
        axis.title.y = element_text(colour="Black", size=16),
